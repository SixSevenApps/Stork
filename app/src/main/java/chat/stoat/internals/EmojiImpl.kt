...
object EmojiRepository {
    ...
    private suspend fun initMetadata(context: Context): List<EmojiGroup> {
        return withContext(Dispatchers.IO) {
            ...
            StoatJson.decodeFromString(ListSerializer(EmojiGroup.serializer()), json)
        }
    }
    ...
    private suspend fun initShortcodeMapping(context: Context): Map<String, String> {
        return withContext(Dispatchers.IO) {
            ...
            StoatJson.decodeFromString(MapSerializer(String.serializer(), String.serializer()), json)
        }
    }
    ...
    fun initialize(scope: CoroutineScope) {
        ...
        scope.launch {
            ...
            try {
                val context = StoatApplication.instance.applicationContext
    ...
    fun serversWithEmotes(): List<Server> {
        val cacheKey = StoatAPI.emojiCache.values.size
    ...
    fun serverEmoteList(server: Server): List<EmojiPickerItem> {
        val cacheKey = "${server.id}_${StoatAPI.emojiCache.values.size}"
    ...
    fun flatPickerList(): List<EmojiPickerItem> {
        ...
        val cacheKey = "${serversWithEmotes().size}_${StoatAPI.emojiCache.values.size}"
