...
object StoatAPI {
            ...
            try {
                withContext(realtimeContext) {
                    try {
                        ...
                        Log.d("StoatAPI", "Socket closed, probably no big deal /// " + e.message)
                        RealtimeSocket.updateDisconnectionState(DisconnectionState.Disconnected)
                    } catch (e: Exception) {
                        Log.e("StoatAPI", "WebSocket error", e)
                        RealtimeSocket.updateDisconnectionState(DisconnectionState.Disconnected)
                    }
                }
            } catch (e: Exception) {
                try {
                    if (e is InterruptedException) {
                        Log.d("StoatAPI", "Socket interrupted")
                    } else {
                        Log.e("StoatAPI", "WebSocket error", e)
                    }
                    RealtimeSocket.updateDisconnectionState(DisconnectionState.Disconnected)
    ...
    fun hydrateFromPersistentCache() {
        if (!openForLocalHydration) {
            Log.w("StoatAPI", "Hydration is closed, but was called")
